...
class Solution:
    ...
    def _getZeroGroups(self, s: str):
        """Returns the zero groups and, for each index, which group (if any) it belongs to."""
        zeroGroups = []
        zeroGroupIndex = []
    ...
    def _getZeroMergeLengths(self, zeroGroups):
        """Combined lengths of every pair of adjacent zero-groups (separated by a single 1)."""
